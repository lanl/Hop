...
for nd in $SRVNODES; do
	...
	ssh -f $ETHP.$nd $HOME/work/go/src/hop/bench/lvldb/run-srv.sh $IBP.$MASTER:5000 $IBP.$nd:5000 $DBDIR/$nd
	sleep 2
done
...
for nd in $CLTNODES; do
	...
	ssh -f $ETHP.$nd $HOME/work/go/src/hop/bench/lvldb/run-clt.sh $IBP.$nd:5000 $D $i $IBP.$MASTER:5000
	i=$((i + 1))
	sleep 2
done
