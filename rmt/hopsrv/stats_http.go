...
func (srv *Srv) statsRegister() {
	once.Do(func() {
		http.HandleFunc("/hop/", StatsHandler)
		go http.ListenAndServe(":6060", nil)
	}
		
	register("/hop/srv/"+srv.Id, srv)
}

func (srv *Srv) statsUnregister() {
	register("/hop/srv/"+srv.Id, nil)
}
...
func (srv *Srv) ServeHTTP(c http.ResponseWriter, r *http.Request) {
	...
	for conn := srv.connlist; conn != nil; conn = conn.next {
		io.WriteString(c, fmt.Sprintf("<a href='/hop/srv/%s/conn/%s'>%s</a><br>", srv.Id, conn.Id, conn.Id))
	}
	srv.Unlock()
}

func (conn *Conn) statsRegister() {
	register("/hop/srv/"+conn.Srv.Id+"/conn/"+conn.Id, conn)
}

func (conn *Conn) statsUnregister() {
	register("/hop/srv/"+conn.Srv.Id+"/conn/"+conn.Id, nil)
}
